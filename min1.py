            ...
            <div class="card shadow-sm">
              <div class="card-body">
                ...
                <div class="d-grid gap-2">
                  <a class="btn btn-outline-primary" href="{ url_for('deposit') }">입금 등록</a>
                  <a class="btn btn-outline-success" href="{ url_for('meal') }">식사 등록</a>
                  <a class="btn btn-outline-dark" href="{ url_for('status') }">현황/정산</a>
                </div>
              </div>
            </div>
          </div>
...
def quick_setup():
    names = []
    for i in range(10):
        v = (request.form.get(f"m{i}") or "").strip()
        if v: names.append(v)
    # 모두 삭제 후 삽입(중복 무시)
    db_execute("DELETE FROM members;")
    for n in names:
        db_execute("INSERT INTO members(name) VALUES (?) ON CONFLICT (name) DO NOTHING;", (n,))
    get_db().commit()
    return redirect(url_for("home"))
...
def meal():
    ...
    for m in members:
        rows_totalcustom += f"""
        <tr>
          <td><input class="form-check-input" type="checkbox" name="ate_{m}"></td>
          <td>{m}</td>
          <td><input class="form-control form-control-sm num total-custom-cell" type="number" name="tot_{m}" min="0" step="1" value="0" disabled></td>
        </tr>"""
        rows_detailed += f"""
        <tr>
          <td><input class="form-check-input" type="checkbox" name="ate_{m}"></td>
          <td>{m}</td>
          <td><input class="form-control form-control-sm num main-custom-cell" type="number" name="main_{m}" min="0" step="1" value="0"></td>
          <td class="side-custom-cell"><input class="form-control form-control-sm num" type="number" name="side_{m}" min="0" step="1" value="0" disabled></td>
        </tr>"""
    payer_options = "<option value=''></option>" + "".join([f"<option value='{n}'>{n}</option>" for n in members])
...
def meal_edit(meal_id):
    ...
    # GET: 기존 값 기반으로 폼 그리기
    parts = db_execute("SELECT name, main_amount, side_amount, total_amount FROM meal_parts WHERE meal_id=? ORDER BY name;", (meal_id,)).fetchall()
    ate_set = {p["name"] for p in parts}
    def checked(m):
        return "checked" if m in ate_set else ""

    # 총액 기반 행
    rows_totalcustom = ""
    for m in members:
        val = next((p["total_amount"] for p in parts if p["name"] == m), 0)
        rows_totalcustom += f"""
        <tr>
          <td><input class="form-check-input" type="checkbox" name="ate_{m}" {checked(m)}></td>
          <td>{m}</td>
          <td><input class="form-control form-control-sm num total-custom-cell" type="number" name="tot_{m}" min="0" step="1" value="{val}"></td>
        </tr>"""

    # 상세 기반 행
    rows_detailed = ""
    for m in members:
        mp = next((p for p in parts if p["name"] == m), None)
        m_main = mp["main_amount"] if mp else 0
        m_side = mp["side_amount"] if mp else 0
        rows_detailed += f"""
        <tr>
          <td><input class="form-check-input" type="checkbox" name="ate_{m}" {checked(m)}></td>
          <td>{m}</td>
          <td><input class="form-control form-control-sm num main-custom-cell" type="number" name="main_{m}" min="0" step="1" value="{m_main}"></td>
          <td class="side-custom-cell"><input class="form-control form-control-sm num" type="number" name="side_{m}" min="0" step="1" value="{m_side}"></td>
        </tr>"""

    payer_options = "<option value=''></option>" + "".join(
        [f"<option value='{n}'{' selected' if n==meal['payer_name'] else ''}>{n}</option>" for n in members]
    )

    body = f"""
    <div class="card shadow-sm">
      <div class="card-body">
        <h5 class="card-title">식사 수정 #{meal_id}</h5>
        <form method="post">
          <div class="mb-2">
            <label class="form-label">날짜</label>
            <input class="form-control" type="date" name="dt" value="{meal['dt']}">
          </div>
          <div class="mb-2">
          ...
          <div class="table-responsive mt-2">
            <table class="table table-sm align-middle">
              <thead><tr><th>식사</th><th>이름</th><th>총액</th></tr></thead>
              <tbody>{rows_totalcustom}</tbody>
            </table>
          </div>
          <!-- 상세 기반 -->
          <div class="table-responsive mt-2">
            <table class="table table-sm align-middle">
              <thead><tr><th>식사</th><th>이름</th><th>메인</th><th>사이드</th></tr></thead>
              <tbody>{rows_detailed}</tbody>
            </table>
          </div>
          <div class="mt-3 d-flex gap-2">
            <button class="btn btn-primary">저장</button>
            <a class="btn btn-outline-secondary" href="{url_for('meal_detail', meal_id=meal_id)}">취소</a>
          </div>
        </form>
      </div>
    </div>
